...
import { formatCurrency, formatDate, simpleHash } from '../lib/utils';
import {
  AlertTriangle,
  X,
  Trash2,
...
export const DeleteInvoiceModal: React.FC<DeleteInvoiceModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full border border-red-200 overflow-hidden my-auto space-y-0">
        {/* Modal Header */}
        <div className="bg-red-700 text-white px-6 py-4 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-red-800 rounded-lg text-red-100 hover:text-white transition cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex border-b border-slate-200 bg-slate-50">
          ...
          <button
            ...
            className={`flex-1 py-3 px-4 text-xs font-bold transition flex items-center justify-center gap-2 border-b-2 cursor-pointer ${
              activeTab === 'delete'
                ? 'border-red-600 text-red-900 bg-red-50/60'
                : 'border-transparent text-slate-600 hover:bg-slate-100'
            }`}
          >
        ...
        <div className="p-6 space-y-5 text-xs text-slate-800">
          ...
          <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 space-y-2">
            <div className="flex justify-between items-center font-bold text-slate-900 border-b border-slate-200 pb-2">
              <span className="font-mono text-blue-600">{transaction.invoiceNumber}</span>
            ...
            <div className="grid grid-cols-2 gap-2 pt-1 text-slate-700">
              ...
              <div>
                ...
                <span className="font-mono font-bold text-slate-900">{formatCurrency(transaction.grandTotal, currency)}</span>
              </div>
              <div>
                <span className="text-slate-400 block text-[10px] uppercase font-bold">Received / Pending</span>
          ...
          {activeTab === 'void' && (
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-slate-300 rounded-xl outline-none focus:ring-2 focus:ring-amber-500 bg-white font-medium text-xs text-slate-900"
              >
          ...
          {passwordHash && (
            <div className="p-3.5 bg-slate-100 rounded-xl border border-slate-200 space-y-2">
              <label className="block text-xs font-bold uppercase text-slate-800 flex items-center gap-1.5">
                <KeyRound className="w-4 h-4 text-red-600" /> Security Authorization Password *
              </label>
              <input
                type="password"
                required
                ...
                className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-red-500 text-sm font-medium"
              />
          ...
          <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition cursor-pointer border border-slate-300 uppercase tracking-wider"
            >
              CANCEL
            </button>
