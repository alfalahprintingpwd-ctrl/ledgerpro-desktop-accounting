import { AppData, BusinessProfile, MonthFile, Transaction, Expense, ExpenseCategory, CashBankTransfer, PasswordRecoveryConfig, SecurityAuditLog, Employee } from '../types';
import { simpleHash, syncAllMonthBalances } from './utils';
...
export const DEFAULT_EMPLOYEES: Employee[] = [
  {
    id: 'emp_1',
    name: 'Muhammad Ali',
    contactNumber: '+92 302 1112233',
    designation: 'Accountant & Operations',
    signatureUrl: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="140" height="50" viewBox="0 0 140 50"><path d="M10 35 Q 25 10 45 30 T 75 15 T 105 35 T 125 25" stroke="%231e3a8a" stroke-width="2" fill="none"/><text x="15" y="45" font-family="sans-serif" font-size="9" fill="%231e3a8a">M. Ali (Accts)</text></svg>',
    status: 'active',
  ...
  },
  {
    id: 'emp_2',
    name: 'Kamran Khan',
    contactNumber: '+92 313 4445566',
    designation: 'Procurement Officer',
    signatureUrl: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="140" height="50" viewBox="0 0 140 50"><path d="M10 25 Q 30 5 55 35 T 85 10 T 115 40" stroke="%231e3a8a" stroke-width="2" fill="none"/><text x="15" y="45" font-family="sans-serif" font-size="9" fill="%231e3a8a">K. Khan</text></svg>',
    status: 'active',
    createdAt: '2026-01-01T00:00:00Z',
  },
  {
  ...
  if (stored) {
    try {
      ...
      if (parsed && typeof parsed === 'object') {
        const rawMonths: MonthFile[] = parsed.months || [];
        const rawTx: Transaction[] = parsed.transactions || [];
        const rawExpenses: Expense[] = (parsed.expenses || []).map((e: any, idx: number) => ({
          ...e,
          voucherNumber: e.voucherNumber || `EXP-2026-${String(idx + 1).padStart(4, '0')}`,
          madeBy: e.madeBy || '',
        }));
        const rawTransfers: CashBankTransfer[] = parsed.transfers || [];

        const syncedMonths = syncAllMonthBalances(rawMonths, rawTx, rawExpenses, rawTransfers);
        ...
        const initialData: AppData = {
          ...
          months: syncedMonths,
          activeMonthId: parsed.activeMonthId || (syncedMonths[syncedMonths.length - 1]?.id || ''),
          transactions: rawTx,
          expenses: rawExpenses,
          employees: Array.isArray(parsed.employees) ? parsed.employees : DEFAULT_EMPLOYEES,
          transfers: rawTransfers,
          customCategories: parsed.customCategories || DEFAULT_CATEGORIES,
        };

        // Immediately sanitize and re-save to clean up any legacy bloated blobs
        try {
          const sanitized = sanitizeAppDataForStorage(initialData);
          localStorage.setItem(STORAGE_KEY, JSON.stringify(sanitized));
        } catch (e) {
          console.warn('Initial storage compaction warning', e);
        }
...
export function sanitizeAppDataForStorage(data: AppData): AppData {
  // 1. Sanitize transactions to ensure they don't carry full duplicated base64 images inside businessSnapshot
  const sanitizedTransactions = (data.transactions || []).map((t) => {
    if (t.businessSnapshot) {
      const { logoUrl, ceoSignatureUrl, businessStampUrl, ...cleanSnapshot } = t.businessSnapshot;
      return {
        ...t,
        businessSnapshot: cleanSnapshot as BusinessProfile,
      };
    }
    return t;
  });

  // 2. Sanitize expenses to remove duplicate base64 signatures from every single line item
  const sanitizedExpenses = (data.expenses || []).map((e) => {
    const clean: Expense = { ...e };
    // Only remove snapshots if they are large base64 strings (fallback to employee signature or businessProfile dynamically)
    if (clean.ceoSignatureSnapshot && clean.ceoSignatureSnapshot.length > 500) {
      delete clean.ceoSignatureSnapshot;
    }
    if (clean.businessStampSnapshot && clean.businessStampSnapshot.length > 500) {
      delete clean.businessStampSnapshot;
    }
    if (clean.employeeSignatureSnapshot && clean.employeeSignatureSnapshot.length > 500) {
      delete clean.employeeSignatureSnapshot;
    }
    return clean;
  });

  // 3. Keep latest 100 security logs
  const sanitizedLogs = (data.securityLogs || []).slice(0, 100);
...
export function saveAppData(data: AppData): boolean {
  try {
    const sanitizedData = sanitizeAppDataForStorage(data);
    const serialized = JSON.stringify(sanitizedData);
    localStorage.setItem(STORAGE_KEY, serialized);
    return true;
  } catch (e) {
    console.warn('Primary save to local storage failed, attempting aggressive compression recovery...', e);
    try {
      // Aggressive Quota Recovery: Strip all snapshots and limit security logs to 20
      const recoveryData: AppData = {
        ...data,
        transactions: (data.transactions || []).map((t) => {
          const { businessSnapshot, ...rest } = t;
          return rest as Transaction;
        }),
        expenses: (data.expenses || []).map((exp) => {
          const {
            employeeSignatureSnapshot,
            ceoSignatureSnapshot,
            businessStampSnapshot,
            ...rest
          } = exp;
          return rest as Expense;
        }),
        securityLogs: (data.securityLogs || []).slice(0, 20),
      };
      localStorage.setItem(STORAGE_KEY, JSON.stringify(recoveryData));
      return true;
    } catch (retryErr) {
      console.error('Critical storage quota exceeded even after aggressive recovery', retryErr);
      return false;
    }
  }
}
...
export function generateSampleData(): AppData {
  ...
  const syncedMonths = syncAllMonthBalances(months, transactions, expenses, transfers);
