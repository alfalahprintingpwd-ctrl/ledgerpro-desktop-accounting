            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-900 text-white font-bold uppercase text-[10px] tracking-wider">
                <tr>
                  ...
                  <th className="py-2.5 px-2">Made By</th>
              ...
              <tbody className="divide-y divide-slate-200 bg-white">
                {monthExpenses.map((exp, index) => (
                  <tr key={exp.id} className="hover:bg-slate-50">
                    <td className="py-2.5 px-2 font-mono text-slate-400 text-center">{index + 1}</td>
                    <td className="py-2.5 px-2 font-medium text-slate-700 whitespace-nowrap">{formatDate(exp.date)}</td>
                    <td className="py-2.5 px-2 font-mono font-bold text-slate-900 text-[10px] whitespace-nowrap">
                      {exp.voucherNumber || `EXP-${exp.date.slice(0, 4)}-0001`}
                    </td>
                    <td className="py-2.5 px-3 font-bold text-slate-900">
                      <div>{exp.title}</div>
                      {exp.description && <div className="text-[10px] text-slate-400 font-normal truncate max-w-xs">{exp.description}</div>}
                    </td>
                    <td className="py-2.5 px-2 text-slate-600 text-[11px] whitespace-nowrap">{exp.category}</td>
                    <td className="py-2.5 px-2 font-semibold text-slate-800 text-[11px] whitespace-nowrap">
                      {exp.madeBy || '-'}
                    </td>
                    <td className="py-2.5 px-2 text-center font-bold">
                      {exp.paymentSource === 'Cash' ? (
                        <span className="text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded text-[10px]">Cash</span>
                      ) : (
                        <span className="text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded text-[10px]">Bank</span>
                      )}
                    </td>
